...
def update_from_temp_collections(conf,no_confirm=False,use_parallel=False):
    t0 = time.time()
    sc = GeneDocSyncer(conf)
    ...
    if no_confirm or ask('Continue?') == 'Y':
        logfile = 'databuild_sync_{}_{}.log'.format(conf, time.strftime('%Y%m%d'))
...
def rename_from_temp_collection(conf,from_index,no_confirm=False):
    ...
    if no_confirm or ask('Continue?') == 'Y':
        bckfile = backup_timestamp_main([conf]).pop()
    else:
        bckfile = None
    ...
    if bckfile is None:
        try:
            pat = "%s_current_tsbk_*.txt.bz" % conf
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        conf = 'mygene_allspecies'
    if not conf.startswith('genedoc_'):
        conf = 'genedoc_' + conf
    assert conf in ['genedoc_mygene', 'genedoc_mygene_allspecies']
    use_parallel = '-p' in sys.argv
    no_confirm = '-b' in sys.argv
    rename = '-r' in sys.argv
    if rename:
        from_index = sys.argv[sys.argv.index('-r') + 1]
        rename_from_temp_collection(conf,from_index,no_confirm)
    else:
        update_from_temp_collections(conf,no_confirm,use_parallel)
