...
def _get_common_parameters(settings, biothing_object, url):
    ...
    _common_params_list = [
        ...
        if (k != "q") and (k != "scopes") and (k != "ids") and (k in getattr(settings, "KWARG_DESCRIPTIONS", {}))
    ]
    # add 'special' parameters not controlled by main biothings web pipeline
    _common_params_list.extend(
                    ...
                    (
                        ...
                        OrderedDict(
                            [
                                ...
                                (
                                    "description",
                                    _description_template_object.get("text_template", "").format(
                                        biothing_object=biothing_object,
                                        param_type=_string_types(_all.get(param, {}).get("type", "")),
                                        param_default_value=_param_default_values(
                                            _all.get(param, {}).get("default", "")
                                        ),
                    ...
                    "properties": OrderedDict(
                        [
                            ...
                            (
                                ...
                                {
                                    "type": "array",
                                    "items": {"$ref": "#/components/schemas/{}".format(biothing_object.title())},
                                },
                            ),
                        ]
                    ),
                    ...
                    "items": {
                        "allOf": [
                            {"$ref": "#/components/schemas/{}".format(biothing_object.title())},
                            {
                                "type": "object",
                                "properties": {
                                    "_score": {"type": "number", "format": "float"},
                                    "query": {"type": "string"},
                                },
                            },
                        ]
...
def _query_get_params(settings):
    _ret = {}
    for d in [
        settings.QUERY_GET_CONTROL_KWARGS,
        settings.QUERY_GET_ES_KWARGS,
        settings.QUERY_GET_ESQB_KWARGS,
        settings.QUERY_GET_TRANSFORM_KWARGS,
    ]:
        for k, v in d.items():
...
def _metadata_params(settings):
    _ret = {}
    for d in [
        settings.METADATA_GET_CONTROL_KWARGS,
        settings.METADATA_GET_ES_KWARGS,
        settings.METADATA_GET_ESQB_KWARGS,
        settings.METADATA_GET_TRANSFORM_KWARGS,
    ]:
        for k, v in d.items():
...
def _annotation_get_params(settings):
    _ret = {}
    for d in [
        settings.ANNOTATION_GET_CONTROL_KWARGS,
        settings.ANNOTATION_GET_ES_KWARGS,
        settings.ANNOTATION_GET_ESQB_KWARGS,
        settings.ANNOTATION_GET_TRANSFORM_KWARGS,
    ]:
        for k, v in d.items():
            if k != "q" and k in getattr(settings, "KWARG_DESCRIPTIONS", {}):
                _ret.setdefault(k, v)
...
def _query_post_params(settings):
    _ret = {}
    for d in [
        settings.QUERY_POST_CONTROL_KWARGS,
        settings.QUERY_POST_ES_KWARGS,
        settings.QUERY_POST_ESQB_KWARGS,
        settings.QUERY_POST_TRANSFORM_KWARGS,
    ]:
        for k, v in d.items():
...
def _annotation_post_params(settings):
    _ret = {}
    for d in [
        settings.ANNOTATION_POST_CONTROL_KWARGS,
        settings.ANNOTATION_POST_ES_KWARGS,
        settings.ANNOTATION_POST_ESQB_KWARGS,
        settings.ANNOTATION_POST_TRANSFORM_KWARGS,
    ]:
        for k, v in d.items():
...
def _query_get_description(settings, biothing_object, url):
    ...
    return OrderedDict(
        [
            ...
            (
                "summary",
                "Make {biothing_object} queries and return matching {biothing_object} hits. Supports JSONP and CORS as well.".format(
                    biothing_object=biothing_object
                ),
            ),
                        ...
                        (
                            ...
                            OrderedDict(
                                [
                                    ...
                                    (
                                        "content",
                                        {"application/json": {"schema": {"$ref": "#/components/schemas/ErrorResult"}}},
                                    ),
                                ]
                            ),
                        ),
...
def _query_post_description(settings, biothing_object, url):
    ...
    return OrderedDict(
        [
            ...
            (
                "summary",
                "Make {biothing_object} batch queries and return matching {biothing_object} hits".format(
                    biothing_object=biothing_object
                ),
            ),
            (
                                        ...
                                        (
                                            ...
                                            OrderedDict(
                                                [
                                                    (
                                                        "description",
                                                        'multiple query terms separated by comma (also "+" or whitespace).  Does not support wildcard queries',
                                                    ),
                                                    ("type", "string"),
                                                ]
                                            ),
                                        ...
                                        (
                                            ...
                                            OrderedDict(
                                                [
                                                    (
                                                        "description",
                                                        _descriptions.get("scopes", {})
                                                        .get("text_template", "")
                                                        .format(param_type="string", param_max=""),
                                                    ),
                                                    ("type", "string"),
                                                ]
                                            ),
                            ...
                            OrderedDict(
                                [
                                    ...
                                    (
                                        ...
                                        {
                                            "application/json": {
                                                "schema": {"$ref": "#/components/schemas/QueryPOSTResult"}
                                            }
                                        },
                                    ),
                                ]
...
def _annotation_get_description(settings, biothing_object, url):
    ...
    return OrderedDict(
        [
            ...
            (
                "summary",
                "Retrieve {biothing_object} objects based on {biothing_object} id.  Supports JSONP and CORS as well.".format(
                    biothing_object=biothing_object
                ),
            ),
            ("parameters", _parameters),
                        ...
                        (
                            ...
                            OrderedDict(
                                [
                                    (
                                        "description",
                                        "A matching {biothing_object} object".format(biothing_object=biothing_object),
                                    ),
                                    (
                                        "content",
                                        {
                        ...
                        (
                            ...
                            OrderedDict(
                                [
                                    (
                                        "description",
                                        "A response indicating an unknown {biothing_object} id".format(
                                            biothing_object=biothing_object
                                        ),
                                    )
                                ]
...
def _annotation_post_description(settings, biothing_object, url):
    ...
    return OrderedDict(
        [
            ...
            (
                "summary",
                "For a list of {biothing_object} ids, return the matching {biothing_object} object".format(
                    biothing_object=biothing_object
                ),
            ),
            (
                                        ...
                                        "properties",
                                        {
                                            "ids": OrderedDict(
                                                [
                                                    (
                                                        "description",
                                                        _descriptions.get("ids", {})
                                                        .get("text_template", "")
                                                        .format(
                                                            biothing_object=biothing_object,
                                                            param_type=" Type: string (list).",
                                                            param_default_value="",
                        ...
                        (
                            ...
                            OrderedDict(
                                [
                                    (
                                        "description",
                                        "A list of matching {biothing_object} objects".format(
                                            biothing_object=biothing_object
                                        ),
                                    ),
                                    (
...
def _metadata_description(settings, biothing_object, url):
    return OrderedDict(
        [
            ...
            (
                "parameters",
                [OrderedDict([("name", "callback"), ("in", "query"), ("$ref", "#/components/parameters/callback")])],
            ),
...
def _metadata_fields_description(settings, biothing_object, url):
    ...
    return OrderedDict(
        [
            ...
            (
                "summary",
                "Get metadata about the data fields available from a {url} {biothing_object} object".format(
                    biothing_object=biothing_object, url=url
                ),
            ),
            ("parameters", _parameters),
    ...
    _doc = OrderedDict(
        [
                ...
                OrderedDict(
                    [
                        (
                            ...
                            kwargs.get(
                                "info__version",
                                "{:.1f}".format(float(getattr(_settings, "API_VERSION", "v1").lstrip("v"))),
                            ),
                        ),
                        (
                            "title",
                            kwargs.get(
                                "info__title", "{} API".format(getattr(_settings, "GA_TRACKER_URL", "MyBiothing.info"))
                            ),
                        ),
                        (
                            "description",
                            kwargs.get(
                                "info__description",
                                getattr(_settings, "KWARG_DESCRIPTION", {}).get(
                                    "_root",
                                    "Documentation of the {url} {biothing_object} query web services.  Learn more about [{url}](http://{url}/)".format(
                                        url=_url, biothing_object=_biothing_object
                                    ),
                                ),
                            ),
                        ),
                ...
                [
                    OrderedDict(
                        [
                            (
                                "url",
                                "http://{url}/{version}".format(
                                    url=_url, version=getattr(_settings, "API_VERSION", "v1")
                                ),
                            ),
            ...
            (
                "tags",
                [
                    {"name": "{biothing_object}".format(biothing_object=_biothing_object)},
                    {"name": "query"},
                    {"name": "metadata"},
                ],
            ),
            (
                ...
                OrderedDict(
                    [
                        (
                            ...
                            OrderedDict(
                                [
                                    (
                                        "get",
                                        _annotation_get_description(
                                            settings=_settings, biothing_object=_biothing_object, url=_url
                                        ),
                                    )
                                ]
                            ),
                        ...
                        # annotation POST
                        (
                            "/{biothing_object}".format(biothing_object=_biothing_object),
                            OrderedDict(
                                [
                                    (
                                        "post",
                                        _annotation_post_description(
                                            settings=_settings, biothing_object=_biothing_object, url=_url
                                        ),
                                    )
                                ]
                        ...
                        (
                            ...
                            OrderedDict(
                                [
                                    (
                                        "get",
                                        _query_get_description(
                                            settings=_settings, biothing_object=_biothing_object, url=_url
                                        ),
                                    ),
                                    (
                                        "post",
                                        _query_post_description(
                                            settings=_settings, biothing_object=_biothing_object, url=_url
                                        ),
                                    ),
                                ]
            ...
            (
                ...
                OrderedDict(
                    [
                        (
                            "parameters",
                            _get_common_parameters(settings=_settings, biothing_object=_biothing_object, url=_url),
                        ),
