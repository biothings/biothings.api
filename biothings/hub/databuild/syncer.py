...
class BaseSyncer(object):
    ...
    def load_metadata(self, diff_folder):
        self._meta = json.load(open(os.path.join(diff_folder,
                                                 "metadata.json")))
        ...
        if "content" in steps:
            if selfcontained:
                ...
                diff_type = diff_type.replace("-selfcontained",
                                              "").replace("-", "_")
            diff_files = [(os.path.join(diff_folder,
                                        e["name"]), e.get("worker_args", {}))
                          for e in self._meta["diff"]["files"]]
            ...
            for diff_file, worker_args in diff_files:
                ...
                strwargs = worker_args and " using specific worker args %s" % repr(
                    worker_args) or ""
                self.logger.info(
                ...
                job = yield from self.job_manager.defer_to_process(
                    ...
                    partial(worker, diff_file, old_db_col_names,
                            new_db_col_names,
                            worker_args.get("batch_size") or batch_size, cnt,
        ...
        if "post" in steps:
            ...
            def posted(f):
                try:
                    res = f.result()
                    self.logger.info(
                        "Post-sync process done on index '%s': %s" %
                        (repr(old_db_col_names), res))
    ...
    def sync(self,
        ...
        job = asyncio.ensure_future(
            self.sync_cols(diff_folder=diff_folder,
                           batch_size=batch_size,
                           mode=mode,
                           target_backend=target_backend,
                           steps=steps,
...
class ThrottledESJsonDiffSelfContainedSyncer(ThrottlerSyncer,
                                             ESJsonDiffSelfContainedSyncer):
    pass


class ThrottledESColdHotJsonDiffSyncer(ThrottlerSyncer,
                                       ESColdHotJsonDiffSyncer):
    pass


class ThrottledESColdHotJsonDiffSelfContainedSyncer(
        ThrottlerSyncer, ESColdHotJsonDiffSelfContainedSyncer):
    pass
...
class SyncerManager(BaseManager):
    ...
    def configure(self, klasses=None):
        ...
        """
        klasses = klasses or [
            MongoJsonDiffSyncer, ESJsonDiffSyncer,
    ...
    def sync(self,
        ...
        if hasattr(btconfig, "SYNC_BATCH_SIZE"):
            ...
            self.logger.debug("Overriding sync batch_size default to %s" %
                              batch_size)
        ...
        if not os.path.exists(diff_folder):
            raise FileNotFoundError(
                "Directory '%s' does not exist, run a diff first" %
                diff_folder)
        ...
        try:
            ...
            self.logger.error("Can't find metadata file in diff folder '%s'" %
                              diff_folder)
            raise
        ...
        try:
            ...
            raise SyncerException("No such syncer (%s,%s) (error: %s)" %
                                  (diff_type, backend_type, e))
