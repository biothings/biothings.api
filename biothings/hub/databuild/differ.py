...
class BaseDiffer(object):
    ...
    async def diff_cols(
        ...
        if mode != "force" and os.path.exists(diff_folder) and "content" in steps:
            if mode == "purge" and os.path.exists(diff_folder):
                ...
                raise FileExistsError(f"Found existing files in '{diff_folder}', use mode='purge'")
        ...
        if os.path.exists(self.metadata_filename):
            ...
            self.metadata = {
                ...
                "old": {"backend": old_db_col_names, "version": self.old.version},
                "new": {"backend": new_db_col_names, "version": self.new.version},
                # when "new" is a mongodb target collection:
                "_meta": {},
                "build_config": {},
            }
        if (
            isinstance(self.new, DocMongoBackend)
            ...
            if not new_doc:
                raise DifferException(
                    f"Collection '{self.new.target_collection.name}' has no corresponding build document"
                )
        ...
        if "mapping" in steps:
            ...
            def diff_mapping(old, new, diff_folder):
                ...
                if old_build and new_build:
                    ...
                    self.logger.info(
                        "Neither '%s' nor '%s' have mappings associated to them, skip",
        ...
        self.logger.info(
            "Summary: (Updated: %s, Added: %s, Deleted: %s, Mapping changed: %s)",
            diff_stats["update"],
            diff_stats["add"],
            diff_stats["delete"],
            diff_stats["mapping_changed"],
        )
        ...
        if "reduce" in steps:
            ...
            async def merge_diff():
                ...
                diff_files = [
                    f
                    for f in glob.glob(os.path.join(diff_folder, "*.pyobj"))
    ...
    def get_metadata(self):
        ...
        if not new_doc:
            raise DifferException(
                "Collection '%s' has no corresponding build document" % self.new.target_collection.name
            )
...
class ColdHotDiffer(BaseDiffer):
    async def diff_cols(self, old_db_col_names, new_db_col_names, *args, **kwargs):
        ...
        new_doc = get_src_build().find_one({"_id": self.new.target_collection.name})
        assert "cold_collection" in new_doc.get("build_config", {}), (
            "%s document doesn't have " % self.new.target_collection.name
        ...
        diff_stat = await super(ColdHotDiffer, self).diff_cols(old_db_col_names, new_db_col_names, *args, **kwargs)
        return diff_stat
    ...
    def get_metadata(self):
        ...
        if not cold_doc:
            raise DifferException(
                "Collection '%s' has no corresponding build document" % self.cold.target_collection.name
            )
...
class ColdHotJsonDifferBase(ColdHotDiffer):
    def post_diff_cols(
        ...
        assert (
            old_doc["build_config"]["cold_collection"] == new_doc["build_config"]["cold_collection"]
        ...
        for diff_file in diff_files:
            ...
            for updt in data["update"]:
                ...
                for patch in updt["patch"]:
                    ...
                    if patch["op"] == "remove" and len(pathk) == 1:
            ...
            if dirty:
                ...
                assert (
                    found
                ), "Couldn't find file information in metadata (with md5 value), try to rebuild_diff_file_list() ?"
...
class ColdHotSelfContainedJsonDiffer(
...
class DiffReportRendererBase(object):
    def __init__(
        ...
        self.max_reported_ids = (
            max_reported_ids or hasattr(btconfig, "MAX_REPORTED_IDS") and btconfig.MAX_REPORTED_IDS or 1000
        )
        self.max_randomly_picked = (
            max_randomly_picked or hasattr(btconfig, "MAX_RANDOMLY_PICKED") and btconfig.MAX_RANDOMLY_PICKED or 10
        )
...
class DifferManager(BaseManager):
    ...
    def get_pinfo(self):
        ...
        (used to report in the hub)
        """
        pinfo = {"category": DIFFMANAGER_CATEGORY, "source": "", "step": "", "description": ""}
        preds = self.get_predicates()
        if preds:
            pinfo["__predicates__"] = preds
        return pinfo
    ...
    def diff_report(
        ...
        def do():
            if mode == "purge" or not os.path.exists(reportfilepath):
                ...
                render = DiffReportTxt(
                    max_reported_ids=max_reported_ids, max_randomly_picked=max_randomly_picked, detailed=detailed
                )
