...
class StandaloneTrackingMixin:
    def self_track(self, event={}):
        no_tracking = self.get_argument('no_tracking', None)
        if not no_tracking and self.web_settings.STANDALONE_TRACKING_URL:
            req = HTTPRequest(self.web_settings.STANDALONE_TRACKING_URL, method='POST', body='category={0}&action={1}'.format(event.get('category', ''), event.get('action', '')))
