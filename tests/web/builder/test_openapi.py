...
def test_implicit_context_change_one_level():
    o = OpenAPIDocumentBuilder()
    c = o.info().server("http://example.org/")
    assert isinstance(c, OpenAPIContext)


def test_implicit_change_multi_level():
    o = OpenAPIDocumentBuilder()
    c = o.path("/api").get().parameter("p", in_="query", required=False).path("/api2")
...
def test_non_allowed_multilevel_raises():
    ...
    with pytest.raises(AttributeError):
        o.path("/api").get().parameter("p", in_="query", required=False).info(title="t")
...
def test_set_server():
    ...
    r = deepcopy(_base_document)
    r["servers"] = [{"url": "http://example.org/api", "description": "Example1"}, {"url": "https://example.org/api"}]
    assert o.document == r
...
def test_operation_init():
    o = OpenAPIDocumentBuilder()
    o.path("/endpoint1", summary="s", description="d").get()
    r = deepcopy(_base_document)
    r["paths"] = {
        "/endpoint1": {
            "summary": "s",
...
def test_parameter_init():
    o = OpenAPIDocumentBuilder()
    o.path("/e/{p}", summary="s", description="d").parameter("p", in_="path", required=True)
    r = deepcopy(_base_document)
    r["paths"] = {
        "/e/{p}": {
            "summary": "s",
...
def test_parameter_type():
    o = OpenAPIDocumentBuilder()
    o.path("/e/{p}", summary="s", description="d").parameter("p", in_="path", required=True).type("integer", default=0)
    r = deepcopy(_base_document)
    r["paths"] = {
        "/e/{p}": {
            "summary": "s",
...
def test_tags():
    o = OpenAPIDocumentBuilder()
    o.path("/api", summary="s", description="d").get().tag("t1").tag("t2")
    r = deepcopy(_base_document)
    r["paths"] = {
        "/api": {
            "summary": "s",
...
def test_method_name_different_field():
    o = OpenAPIDocumentBuilder()
    o.path("/api", summary="s", description="d").get().operation_id("opid")
    r = deepcopy(_base_document)
    r["paths"] = {
        "/api": {
            "summary": "s",
