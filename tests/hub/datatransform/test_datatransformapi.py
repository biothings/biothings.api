...
def test_inchi_to_inchikey():
    ...
    res_lst = load_document(doc_lst)
    res = next(res_lst)
    assert res["_id"] == "USNINKBPBVKHHZ-CYUUQNCZSA-L"
    res = next(res_lst)
...
def test_input_source_fields():
    ...
    res_lst = load_document("data/folder/")
    r = next(res_lst)
    assert r["_id"] == "RZVAJINKPMORJF-UHFFFAOYSA-N"
    r = next(res_lst)
    assert r["_id"] == "XUKUURHRXDUEBC-KAYWLYCHSA-N"
    r = next(res_lst)
    assert r["_id"] == "XGZVUEUWXADBQD-UHFFFAOYSA-L"
    r = next(res_lst)
    assert r["_id"] == "HEFNNWSXXWATRW-UHFFFAOYSA-N"


@pytest.mark.xfail
def test_long_doc_lst():
    """
